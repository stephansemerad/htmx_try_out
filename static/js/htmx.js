  ...
  (function () {
    ...
    function getTarget(elt) {
      var explicitTarget = getClosestMatch(elt, function (e) {
        return getRawAttribute(e, "hx-target") !== null;
      });
      if (explicitTarget) {
      ...
      forEach(toArray(fragment.children), function (child) {
        if (getAttributeValue(child, "hx-swap-oob") === "true") {
          ...
          if (target) {
            ...
            triggerErrorEvent(getDocument().body, "oobErrorNoTarget.htmx", {
              content: child,
            });
          }
        }
    ...
    function swapOuterHTML(target, fragment) {
      if (target.tagName === "BODY") {
        ...
        var settleTasks = insertNodesBefore(
          parentElt(target),
          target,
          fragment
        );
    ...
    function swapResponse(swapStyle, target, elt, responseText) {
      ...
      if (fragment) {
        ...
        switch (swapStyle) {
          ...
          default:
            return concat(settleTasks, swapInnerHTML(target, fragment));
        }
      }
    }
            ...
            if (classOperation.operation === "toggle") {
              setTimeout(function () {
                setInterval(function () {
                  elt.classList[classOperation.operation].call(
                    elt.classList,
                    classOperation.cssClass
                  );
    ...
    function initScrollHandler() {
      if (!window["htmxScrollHandler"]) {
        var scrollHandler = function () {
          forEach(
            getDocument().querySelectorAll("[hx-trigger='revealed']"),
            function (elt) {
              maybeReveal(elt);
            }
          );
        ...
        var sseListener = function () {
          if (!maybeCloseSSESource(sseSource)) {
            if (bodyContains(elt)) {
              ...
              sseSource.sseSource.removeEventListener(
                sseEventName,
                sseListener
              );
            }
    ...
    function sendError(elt, eventName, detail) {
      ...
      if (errorURL) {
        ...
        xhr.send(
          JSON.stringify({ elt: elt.id, event: eventName, detail: detail })
        );
      }
    }
    ...
    function saveHistory() {
      ...
      triggerEvent(getDocument().body, "beforeHistorySave.htmx", {
        path: path,
        historyElt: elt,
      });
    ...
    function loadHistoryFromServer(path) {
      ...
      request.onload = function () {
        if (this.status >= 200 && this.status < 400) {
          ...
          triggerErrorEvent(
            getDocument().body,
            "historyCacheMissLoadError.htmx",
            details
          );
        }
      };
    ...
    function restoreHistory(path) {
      ...
      if (cached) {
        settleImmediately(
          swapInnerHTML(getHistoryElement(), makeFragment(cached.content))
        );
    ...
    function shouldPush(elt) {
      return (
        getClosestAttributeValue(elt, "hx-push-url") === "true" ||
    ...
    function getHeaders(elt, target, prompt, eventTarget) {
      ...
      if (getDocument().activeElement) {
        headers["X-HX-Active-Element"] = getRawAttribute(
          getDocument().activeElement,
          "id"
        );
        headers["X-HX-Active-Element-Name"] = getRawAttribute(
          getDocument().activeElement,
          "name"
        );
        if (getDocument().activeElement.value) {
          headers["X-HX-Active-Element-Value"] = getRawAttribute(
            getDocument().activeElement,
            "value"
          );
        }
      ...
      if (info) {
        ...
        if (split.length > 0) {
          ...
          for (var i = 1; i < split.length; i++) {
            var modifier = split[i];
            if (
              modifier.indexOf("encoding:") === 0 &&
              modifier.indexOf("json") >= 0
            ) {
    ...
    function issueAjaxRequest(elt, verb, path, eventTarget) {
      ...
      if (target == null) {
        triggerErrorEvent(elt, "targetError.htmx", {
          target: getRawAttribute(elt, "hx-target"),
        });
        return;
      }
      ...
      if (!triggerEvent(elt, "configRequest.htmx", requestConfig))
        return endRequestLock();
      ...
      if (verb === "get") {
        ...
        requestURL =
          urlSpec.url + (noValues ? "" : "?" + urlEncode(filteredParameters));
      ...
      xhr.onload = function () {
        try {
          ...
          var hxTrigger =
            this.getAllResponseHeaders().indexOf("X-HX-Trigger") >= 0
              ? this.getResponseHeader("X-HX-Trigger")
              : null;
          handleTrigger(elt, hxTrigger);

          var pushedUrl =
            this.getAllResponseHeaders().indexOf("X-HX-Push") >= 0
              ? this.getResponseHeader("X-HX-Push")
              : null;
      ...
      if (!triggerEvent(elt, "beforeRequest.htmx", eventDetail))
        return endRequestLock();
